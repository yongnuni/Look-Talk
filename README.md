...
└── src/
    ...
    │   │                            #   세 트리거(dwell/깜빡임/입벌림) 공용 hover 해석 지점 — fixation.py를 여기서 물린다
    │   ├── eye_tracking.py          # MediaPipe 홍채 좌표 계산, 눈 깜빡임(EAR) 검출, 홍채 추적 신뢰도(iris_confidence), 눈/홍채 시각화
    │   ├── fixation.py              # 고정 감지형 히트박스 확장 — I-VT(속도)+I-DT(분산)로 고정 시작을 독자 판정하고,
    │   │                            #   그 키의 판정 영역만 키 크기에 비례해 넓힌다 (화면상 키 크기·시선 좌표는 불변)
    │   │                            #   인접 키를 최대 1/3까지 덮고, 겹치는 구간에서는 확장된 키가 우선
    │   │                            #   고정된 키캡은 화면에서도 짧은 ease-out으로 확대된다(ui.drawAll에 확대 사각형 전달).
    │   │                            #   주변 키캡은 밀리지 않고 정적으로 유지되며, 판정 영역이 시각 확대보다 조금 더 넓다
